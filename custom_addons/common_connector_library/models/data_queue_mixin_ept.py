...
class DataQueueMixinEpt(models.AbstractModel):
            ...
            try:
                ...
                for tbl_name in queue_detail:
                    ...
                    if is_delete_queue:
                        ...
                        continue
                    query = sql.SQL("delete FROM {} where cast(create_date as Date) <= current_date - %s").format(sql.Identifier(tbl_name))
                    params = (7,)
    ...
    def delete_data_queue_schedule_activity_ept(self, tbl_name, is_delete_queue=False):
        ...
        if is_delete_queue:
            ...
            query = sql.SQL("select id from {} where cast(create_date as Date) <= current_date - %s").format(sql.Identifier(tbl_name))
            params = (7,)
