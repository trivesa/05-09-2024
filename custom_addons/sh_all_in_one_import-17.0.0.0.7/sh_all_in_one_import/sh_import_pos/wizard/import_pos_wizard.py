                                ...
                                if sheet.cell(row, 0).value not in (None, "") and sheet.cell(row, 5).value not in (None, ""):
                                    ...
                                    if created_pos:
                                        ...
                                        if search_product:
                                            ...
                                            price_subtotal = float(sheet.cell(row, 9).value)
                                            price_subtotal_incl = float(sheet.cell(row, 9).value)
                                            if vals.get('tax_ids', False):
                                                if vals.get('tax_ids')[0] and vals.get('tax_ids')[0][2] not in (None, ''):
                                                    taxObj = self.env['account.tax'].search([('id', 'in', vals.get('tax_ids')[0][2])])
                                                    compute_tax = taxObj.compute_all(float(sheet.cell(row, 9).value), product=search_product.product_tmpl_id, partner=self.env['res.partner'])
                                                    price_subtotal = compute_tax['total_excluded']
                                                    price_subtotal_incl = compute_tax['total_included']

                                            vals.update({'price_subtotal': float(price_subtotal) * float(sheet.cell(
                                                row, 7).value), 'price_subtotal_incl': float(price_subtotal_incl) * float(sheet.cell(row, 7).value)})
                                            vals.update({'order_id': created_pos.id})
                                            line = pos_line_obj.create(vals)
                                            line.order_id._compute_batch_amount_all()
                                            line.order_id._onchange_amount_all()
                                            line.order_id._compute_order_name()
