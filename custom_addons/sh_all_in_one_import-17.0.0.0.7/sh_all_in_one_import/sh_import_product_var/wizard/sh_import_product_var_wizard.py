...
#                                             continue
                                    ...
                                    if row[14].strip() in (
                                        ...
                                        if row[17] not in (None, ""):
                                            tmpl_vals.update(
                                                {'barcode': row[17]})
                                        if row[18] not in ("",False,None,0,0.0,'0','0.0'):
                                            tmpl_vals.update(
                                                {'weight': row[18]})
                                        if row[19] not in ("",False,None,0,0.0,'0','0.0'):
                                            tmpl_vals.update(
                                                {'volume': row[19]})
                                ...
                                if created_product_tmpl and has_variant:
                                    ...
                                    if row[14].strip() not in (
                                        ...
                                        if self.method == 'create':
                                            ...
                                            if created_product_tmpl.product_variant_ids:
                                                ...
                                                if row[22] not in ("",False,None,0,0.0,'0','0.0'):
                                                    var_vals.update({
                                                        'standard_price':float(row[22])
                                                    })
                                    ...
                                    if created_product_tmpl:
                                        ...
                                        if is_any_error_in_dynamic_field:
                                            counter = counter + 1
                                            continue                                        
                                        created_product_tmpl.write(dynamic_vals)
                                counter = counter + 1
                            else:
                                skipped_line_no[str(
                                    counter
