...
class ImportProductVarMBWizard(models.TransientModel):
        ...
        if field_required and field_value in (None, ""):
            ...
            if field_value.strip() not in (None, ""):
                for x in field_value.split(','):
                    x = x.strip()
                    if x != '':
                        record = self.env[name_relational_model].sudo().search([(field_name_m2o, '=', x)], limit=1)
    ...
    def validate_field_many2one(self, field_name, field_ttype, field_value,field_required, field_name_m2o):
        ...
        if field_required and field_value in (None, ""):
            ...
            record = self.env[name_relational_model].sudo().search([(field_name_m2o, '=', field_value)], limit=1)
                            ...
                            if row[0] not in (None,"") and row[1] not in (None, ""):
                                ...
                                if row[0] != running_tmpl:
                                    ...
                                    if self.product_update_by == 'name':
                                        running_tmpl_domain = [('name', '=', row[1]),]

                                    elif self.product_update_by == 'barcode':
                                        running_tmpl_domain = [('product_variant_ids.barcode','=', row[17]),]

                                    elif self.product_update_by == 'int_ref':
                                        running_tmpl_domain = [('product_variant_ids.default_code','=', row[16]),]
                                    ...
                                    if row[4].strip() not in (None, "") and row[4].strip() == 'Service':
                                        product_type = 'service'
                                        tmpl_vals.update({'type': product_type})
                                    elif row[4].strip() not in (None, "") and row[4].strip() == 'Storable Product' or row[4].strip() == 'Stockable Product':
                                    ...
                                    if row[13] not in ("",False,None,0,0.0,'0','0.0'):
                                        tmpl_vals.update(
